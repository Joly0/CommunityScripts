...
if not os.path.isdir(venv_dir) and not (sys.version_info.major == 3 and sys.version_info.minor >= 10):
    ...
    log.error("Error: Python version must be >= 3.10.X for the first installation of the plugin. Once installed you can change back your python version in stash as this plugin will run within its own venv")
    log.error(f"Current version: {sys.version}")
    log.error("Go to https://www.python.org/downloads/")
    sys.exit(1)
